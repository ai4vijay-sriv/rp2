...
run_name = f"cql/single/{datetime.now().strftime('%Y%m%d-%H%M%S')}"
...
for episode in range(num_episodes):
    ...
    while not done:
        if np.random.rand() < eps:
            ...
            action = int(np.argmax([q_value_v(state, a) for a in range(n_actions)]))
        ...
        next_action1 = int(np.argmax([q_value_u(next_state, a) for a in range(n_actions)]))
...
print(avg_ret)
writer.add_scalar("summary/avg_return", avg_ret, num_episodes - 1)
writer.close()
