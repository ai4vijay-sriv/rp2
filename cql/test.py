...
embedded_dim = feature_dim + n_actions
weights_u = np.zeros(embedded_dim, dtype=np.float32)
weights_v = np.zeros(embedded_dim, dtype=np.float32)

# Embedded feature vector with action as one-hot, normalized to L2 norm = 1
def psi(state, action):
    state_feat = phi(state)
    one_hot = np.zeros(n_actions, dtype=np.float32)
    one_hot[action] = 1.0
    full_feat = np.concatenate([state_feat, one_hot])
    return full_feat / (np.linalg.norm(full_feat) + 1e-8)
...
# Q-value: single weight vector
def q_value_u(state, action):
    return np.dot(weights_u, psi(state, action))

def q_value_v(state, action):
    return np.dot(weights_v, psi(state, action))
...
for episode in range(args.num_episodes):
    ...
    total_reward = 0
    epsilon = get_epsilon(episode) # if you want schedule
    ...
    while not done:
        ...
        # Gradient update
        psi_vec = psi(state, action)
        weights_u += args.alpha * q_value_v(state, action) * psi_vec
        weights_v += args.beta * td_error * psi_vec

        weights_u = np.clip(weights_u, -args.weight_clip_value, args.weight_clip_value)
        weights_v = np.clip(weights_v, -args.weight_clip_value, args.weight_clip_value)
    ...
    writer.add_scalar("weight_u_mean", weights_u.mean(), episode)
    writer.add_scalar("weight_v_mean", weights_v.mean(), episode)
